...
import glob
import os
from halogen.parser import get_file_hash
from halogen.generator import yara_image_rule_maker
from halogen.render import yara_print_rule
...
-----------------------------------------------
    ...
    def run(self):
        ...
        self.get_file = get_file_hash(self)
    ...
    def dir_run(self):
        ...
        returns: combo list. """
        file_list = glob.glob(self.dir + "/*")
        combo = []
        for file_name in file_list:
            if os.path.isfile(file_name):
                self.image_name = None
                self.yara_base_file = file_name
                self.get_file = get_file_hash(self)
                self.dir_hash.append(self.get_file[0])
                rule_dict = yara_image_rule_maker(self)
                if rule_dict is not None:
                    for i in rule_dict:
                        if i not in combo:
